...
from pretix.presale.views.event import get_grouped_items, item_group_by_category
...
def control_nav_import(sender, request=None, **kwargs):
    ...
    if request.event.has_subevents:
            return [{
            'label': _('Seating'),
            'url': reverse('plugins:pretix_seatingplan:event.seating.upload', kwargs={
            ...
            'children': [
                {
                    ...
                    'url': reverse('plugins:pretix_seatingplan:event.seating.subevents', kwargs={
                        'event': request.event.slug,
                        'organizer': request.event.organizer.slug
                    })
                },
            ...
            'children': [
                {
                    ...
                    'url': reverse('plugins:pretix_seatingplan:event.seating.display', kwargs={
                        'event': request.event.slug,
                        'organizer': request.event.organizer.slug
                    })
                },
...
def render_seating_plan(sender,request, **kwargs):
    ...
    if 'category_seat' in request.GET :
        ...
        print("cat_seat", category_seat)


        # What if cats is empty ?
    ...
    rel_dir = 'templates/pretix_seatingplan/seatingplan.html'
    abs_file_path = os.path.join(script_dir, rel_dir)

    f = open(abs_file_path) # This looks hacky
    content = f.read()
    template = Template(content)

    url_reverse = eventreverse(request.event,'plugins:pretix_seatingplan:event.seating.displaydata')
    url_reverse_assign = eventreverse(request.event,'plugins:pretix_seatingplan:event.seating.productassign')
    ...
    if subevent is not None:
        ...
        url_reverse_assign = eventreverse(request.event,'plugins:pretix_seatingplan:event.seating.productassign',kwargs={
            'subevent':subevent.id
        })
    if 'category_seat' in request.GET :
        product = True
    ...
    context = Context({
            'active_prod': product,
            'items_by_category_seats': item_group_by_category(items_final) ,
            'seat_guid': seat_guid,
            'url_data': url_reverse,
            'url_data_assign': url_reverse_assign
        })
